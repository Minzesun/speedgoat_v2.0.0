...
verifyTrue(testCase, contains(runbook, 'SGV2_MAX_TRACKING_SPEED'));
verifyFalse(testCase, contains(runbook, 'position_rate_command_6064'));
...
verifyTrue(testCase, contains(reference, '固定为 `0`'));
verifyTrue(testCase, contains(reference, 'time, position_command_6064'));
...
verifyFalse(testCase, contains(tuning, 'SGV2_POSITION_RATE_COMMAND_6064'));
...
verifyTrue(testCase, contains(tuning, '不是轨迹规划速度'));
...
verifyTrue(testCase, contains(logic, 'PID-only'));
verifyTrue(testCase, contains(logic, '只是速度上限'));
