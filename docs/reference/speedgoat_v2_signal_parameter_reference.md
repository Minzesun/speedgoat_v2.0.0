...
| `data/reference/position_reference_6064.txt` | 一列位置 reference 文件；每行一个控制周期的位置点 | Operator txt file | 构建前编辑，构建时自动读入模型 | PT-8 调参 runbook |
...
| `position_reference_6064` | txt 文件播放出来的位置 reference | Position Reference Source | 在 Data Inspector 里和 `position_actual_6064` 叠加看 | txt reference 文件和模型采样时间 |
...
位置跟踪、逆模型和位置 PID 已经进入 PT-5 模型。位置环请求在模型内部固定开启，实际运动仍由 `ready_to_run`、PT-5 限幅和启动控制器门禁保护。操作流程同步记录在项目根目录的 `SPEEDGOAT_V2_MINIMAL_LOGIC.md` 第 15 节；后续如果加新的 tunable 或观测信号，这里要同步补表。
